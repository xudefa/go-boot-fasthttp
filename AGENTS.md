# go-boot-fasthttp 项目开发规范文档

go-boot-fasthttp 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 FastHTTP 高性能 HTTP 客户端集成模块。本模块将 FastHTTP 框架无缝集成到 go-boot 的 IoC 容器和自动配置体系中,遵循 go-boot 项目的开发规范。
...
- **集成模块**:go-boot-fasthttp 是 go-boot 的 HTTP 客户端集成,将 FastHTTP 作为高性能 HTTP Client 实现
- **规范继承**:完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 FastHTTP Client 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 FastHTTP 启动器
- 实现 `net.HttpClient` 接口的 FastHTTP 适配器
- 支持连接池管理、请求选项配置的声明式配置
...
- **HTTP 框架**:集成 FastHTTP(`github.com/valyala/fasthttp`)
- **职责边界**:仅负责 FastHTTP Client 集成,不包含其他业务逻辑
- **示例代码**:统一放在 `examples/` 目录,演示 FastHTTP 集成用法

### 2.2 go-boot-fasthttp 核心包结构

| 包 | 说明 | 核心类型/函数 |
|---|------|----------|
| `fasthttp/` | FastHTTP Client 集成(自动配置、Bean 注册、HTTP 请求) | `HttpClient`, `NewHttpClient`, `WithBaseURL`, `WithTimeout`, `FasthttpAutoConfiguration` |

### 2.3 go-boot 核心包参考

go-boot-fasthttp 依赖 go-boot 的以下核心包:
...
| `net/` | HTTP 服务器/客户端抽象接口 | `net.HttpClient`, `net.HttpResponse` |
| `tracing/` | 分布式追踪抽象 + LocalTracer 实现 | `tracing.Tracer`, `tracing.Span` |

### 2.4 接口抽象原则

go-boot-fasthttp 遵循 go-boot 的接口抽象原则,所有集成层通过核心框架中的接口抽象定义,实现运行时互换:

- `net.HttpClient` — HTTP 客户端
...
func init() {
    boot.RegisterAutoConfig(
        &FasthttpAutoConfiguration{},
        condition.OnProperty("fasthttp.enabled", "true"),
    )
}
```
